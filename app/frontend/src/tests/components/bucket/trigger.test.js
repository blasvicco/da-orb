...
describe('BucketTrigger', () => {
  ...
  it('opens the native file dialog when the upload button is clicked', async () => {
    mount(BucketTrigger, { props: { open: true, sessionId: 42 } });
    await flushPromises();
    const [fileInput] = await waitForBody('.orb-bucket-file-input', 1);
    const clickSpy = vi.spyOn(fileInput.element, 'click');

    await body().find('.orb-bucket-upload-btn').trigger('click');
  ...
  ])('uploads nothing when the file dialog reports %s', async (_label, files) => {
    mount(BucketTrigger, { props: { open: true, sessionId: 42 } });
    await flushPromises();
    const [fileInput] = await waitForBody('.orb-bucket-file-input', 1);
  ...
  it.each([
    ['closes', false, false],
    ['stays open', true, true],
  ])('the delete confirm %s when the popconfirm reports openChange %s', async (_label, isOpen, expectedOpen) => {
    mockBucket.files.mockResolvedValue([makeFile({ id: 1 })]);
    const wrapper = mount(BucketTrigger, { props: { open: true, sessionId: 42 } });
    await flushPromises();
    const [deleteButton] = await waitForBody('.orb-bucket-delete-btn', 1);
    await deleteButton.trigger('click');
    const popconfirm = wrapper.findComponent({ name: 'APopconfirm' });
    expect(popconfirm.props('open')).toBe(true);

    await popconfirm.vm.$emit('openChange', isOpen);

    expect(popconfirm.props('open')).toBe(expectedOpen);
  });

  it.each([
    ['a user upload', 'user_upload', 'Uploaded by you'],
    ['an assistant-generated file', 'workflow_generated', 'Generated by the assistant'],
  ])('labels %s with where it came from', async (_label, origin, expected) => {
    mockBucket.files.mockResolvedValue([makeFile({ origin })]);
    mount(BucketTrigger, { props: { open: true, sessionId: 42 } });
    await flushPromises();

    const [meta] = await waitForBody('.orb-bucket-item-meta', 1);

    expect(meta.text()).toContain(expected);
  });
  ...
  ])('stops loading and shows nothing to preview when the download URL comes back as %s', async (_label, result) => {
    mockBucket.files.mockResolvedValue([makeFile({ mime_type: 'image/png', name: 'logo.png' })]);
    mockBucket.downloadUrl.mockResolvedValue(result);
    mount(BucketTrigger, { props: { open: true, sessionId: 42 } });
    await flushPromises();

    const [fileName] = await waitForBody('.orb-bucket-item-name', 1);
    await fileName.trigger('click');
    await flushPromises();

    expect(body().find('.orb-bucket-preview-loading').exists()).toBe(false);
    expect(body().find('.orb-bucket-preview-image').exists()).toBe(false);
    expect(body().find('.orb-bucket-preview-text').text()).toBe('');
    expect(globalThis.fetch).not.toHaveBeenCalled();
  });

  it('lets the preview modal close itself', async () => {
    mockBucket.files.mockResolvedValue([makeFile({ mime_type: 'image/png', name: 'logo.png' })]);
    mockBucket.downloadUrl.mockResolvedValue({ url: 'https://signed.example/logo.png' });
    const wrapper = mount(BucketTrigger, { props: { open: true, sessionId: 42 } });
    await flushPromises();
    ...
    await wrapper.findComponent({ name: 'AModal' }).vm.$emit('update:open', false);

    expect(wrapper.findComponent({ name: 'AModal' }).props('open')).toBe(false);
  });

  it.each([
    ['a null mime type', null],
    ['no mime type', undefined],
  ])('treats a file with %s as not previewable', async (_label, mimeType) => {
    mockBucket.files.mockResolvedValue([makeFile({ mime_type: mimeType })]);
    const wrapper = mount(BucketTrigger, { props: { open: true, sessionId: 42 } });
    await flushPromises();
    const [fileName] = await waitForBody('.orb-bucket-item-name', 1);
    ...
    expect(fileName.classes()).not.toContain('orb-bucket-item-name--clickable');
    expect(wrapper.findComponent({ name: 'AModal' }).props('open')).toBe(false);
  ...
  it.each([
    ...
    ['a blank url', { url: '' }],
    ['no result at all', undefined],
  ])('opens nothing when the download URL comes back as %s', async (_label, result) => {
    mockBucket.files.mockResolvedValue([makeFile()]);
    mockBucket.downloadUrl.mockResolvedValue(result);
    const openSpy = vi.spyOn(window, 'open').mockImplementation(() => null);
    mount(BucketTrigger, { props: { open: true, sessionId: 42 } });
    await flushPromises();
    const [downloadButton] = await waitForBody('.orb-bucket-download-btn', 1);
